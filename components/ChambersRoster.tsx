...
type Row = { name: string; designation: string; role?: string; photo?: string; href: string };
...
export default function ChambersRoster() {
  ...
  const groups = teamGroups.map((g) => {
    ...
    if (g.id === "partners") members.unshift({ ...principal, photo: principal.image, href: "/associates#principal" });
    return { ...g, members };
  });
  ...
  return (
    ...
    >
      <AnimatePresence>
        {active && (
          <motion.div
            ...
            className={`pointer-events-none absolute z-20 hidden [@media(hover:hover)]:block overflow-hidden shadow-[0_24px_60px_rgba(10,13,19,0.3)] ${
              active.photo ? "w-40 h-50" : "w-28 h-28 rounded-full bg-gold"
            }`}
          >
            <AnimatePresence initial={false} mode="popLayout">
              ...
              >
                {active.photo ? (
                  <Image src={active.photo} alt="" fill sizes="160px" className="object-cover object-top" />
                ) : (
                  <span className="font-display font-semibold text-[34px] tracking-[-0.05em] text-ink">
      ...
      {groups.map((g) => (
        <div key={g.id} className="grid grid-cols-1 md:grid-cols-[220px_minmax(0,1fr)] gap-x-10 border-t border-ink/15">
          <div className="pt-6 pb-3 md:pb-6 flex md:flex-col justify-between md:justify-start gap-2">
            <span className="font-medium text-[10.5px] tracking-[0.18em] uppercase text-ink/62">{g.label}</span>
            <span className="font-medium text-[10.5px] text-gold-deep">{g.members.length}</span>
          </div>
              ...
              return (
                ...
                >
                  <Link
                    ...
                    className={`group grid grid-cols-[minmax(0,1fr)_auto] sm:grid-cols-[minmax(0,1fr)_minmax(0,220px)_24px] items-center gap-x-4 py-4.5 text-ink transition-opacity duration-300 ${
                      active && active.name !== m.name ? "[@media(hover:hover)]:opacity-35" : ""
                    }`}
                  >
                    ...
                    <span className="hidden sm:flex flex-col items-end text-right gap-1">
                      ...
                      {m.role && (
                        <span className="font-medium text-[9.5px] tracking-[0.16em] uppercase text-gold-deep">{m.role}</span>
                      )}
                    </span>
